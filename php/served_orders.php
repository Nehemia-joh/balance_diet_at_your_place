...
$vendor_orders="SELECT * FROM cus_order where vendor_id='$vendor_id' and statues='Served'";
...
echo "<tr class=\"header\">
      ...
      while($row_vendor_order= mysqli_fetch_array($vendor_order_query)){
        ...
        if($row_vendor_order['food_time']=='breakfast'){
        ...
        else if($row_vendor_order['food_time']=='lunch'){
            $vendor_name="SELECT * FROM lunch where food_id='$food_id1'";
            $food_name_query=mysqli_query($link,$vendor_name);
            while($row_food_name= mysqli_fetch_array($food_name_query)){
            $food_name1=$row_food_name['how_it_read'];
            }
        }
        else if($row_vendor_order['food_time']=='dinner'){
            $vendor_name="SELECT * FROM dinner where food_id='$food_id1'";
            $food_name_query=mysqli_query($link,$vendor_name);
            while($row_food_name= mysqli_fetch_array($food_name_query)){
            $food_name1=$row_food_name['how_it_read'];
            }
        }
        else if($row_vendor_order['food_time']=='snacks'){
            $vendor_name="SELECT * FROM snacks where food_id='$food_id1'";
            $food_name_query=mysqli_query($link,$vendor_name);
            while($row_food_name= mysqli_fetch_array($food_name_query)){
            $food_name1=$row_food_name['how_it_read'];
            }
